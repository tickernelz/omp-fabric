...
export class LcmMaintenance {
  ...
  selectUpgrades(sessionId?: string, activeSources?: ReadonlySet<string>, limit = 1): LcmNode[] {
    const all = this.listNodes(100000);
    const taint = this.tainted(all);
    return all
      .filter(node => !taint.has(node.nodeId) && node.state === "ready" && node.modelHash === "emergency" && node.policyHash === this.policyHash
  ...
  /** The oversized deterministic leaves and every ancestor that inherited one. */
  private tainted(nodes: readonly LcmNode[]): ReadonlySet<string> {
    const oversized = nodes.filter(node => node.kind === "leaf" && node.modelHash === "emergency" && node.sources.length > Math.max(this.maxLeaf, DEFAULT_LEAF_ENTRIES)).map(node => node.nodeId);
    if (oversized.length === 0) return EMPTY_TAINT;
    const parents = new Map<string, string[]>();
    for (const node of nodes) for (const child of node.children) parents.set(child, [...(parents.get(child) ?? []), node.nodeId]);
