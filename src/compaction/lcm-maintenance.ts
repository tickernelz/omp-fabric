...
export interface LcmMaintenanceOptions { now?: () => number; ownerId?: string; policyHash?: string; maxLeafEntries?: number; maxCondenseChildren?: number; maxInputChars?: number; maxOutputChars?: number; modelTimeoutMs?: number; budget?: Partial<LcmBudgetPolicy>; }
...
export class LcmMaintenance {
  ...
  constructor(private readonly ledger: LcmLedger, options: LcmMaintenanceOptions = {}) { this.projectKey = ledger.project.key; this.now = options.now ?? Date.now; this.ownerId = options.ownerId ?? crypto.randomUUID(); this.policyHash = options.policyHash ?? hash("lcm-policy-v1"); this.maxLeaf = options.maxLeafEntries ?? DEFAULT_LEAF_ENTRIES; this.maxChildren = options.maxCondenseChildren ?? 4; this.maxInputChars = options.maxInputChars ?? 200_000; this.maxOutputChars = options.maxOutputChars ?? 4_096; this.modelTimeoutMs = options.modelTimeoutMs ?? 120_000; this.budget = { ...DEFAULT_LCM_BUDGET, ...options.budget }; }
  ...
  claimableJobs(sessionId?: string, limit = 64): LcmJob[] { const t = this.now(); const scope = sessionId ?? null; return this.ledger.readOnly(db => (db.prepare(CLAIMABLE_JOBS_SQL).all(this.projectKey, t, t, t, scope, scope, limit) as Array<{payload:string}>).map(r => { const job = parse<LcmJob>(r.payload); if (job.projectKey !== this.projectKey) throw new Error("job project does not match ledger project"); return job; })); }
  ...
  isClaimable(job: LcmJob): boolean { const t = this.now(); return (job.state === "pending" || (job.state === "running" && (job.leaseUntil ?? 0) <= t)) && job.eligibleAt <= t && job.nextRetryAt <= t; }
  sweepExpiredLeases(graceMs = 0): LcmJob[] { return this.ledger.transaction(db => { const t = this.now(); const rows = db.prepare("SELECT payload FROM maintenance_jobs WHERE project_key=? AND status=?").all(this.projectKey, "running") as Array<{payload:string}>; const swept: LcmJob[] = []; for (const row of rows) { const old = parse<LcmJob>(row.payload); if (old.projectKey !== this.projectKey) throw new Error("job project does not match ledger project"); if ((old.leaseUntil ?? 0) + graceMs > t) continue; const { ownerId: _ownerId, leaseToken: _leaseToken, leaseUntil: _leaseUntil, ...base } = old; const job: LcmJob = { ...base, state: "pending", error: "lease expired", eligibleAt: t, nextRetryAt: t, updatedAt: t }; db.prepare("UPDATE maintenance_jobs SET status=?,payload=?,updated_at=? WHERE job_id=? AND project_key=?").run(job.state, JSON.stringify(job), job.updatedAt, job.jobId, this.projectKey); swept.push(job); } return swept; }); }
  ...
  withinBudget(sessionId: string): boolean { return this.ledger.readOnly(db => { const day = this.day(this.now()); const project = this.usage(db, day); const session = this.usage(db, day, sessionId); return project.calls < this.budget.calls && project.inputTokens < this.budget.inputTokens && project.outputTokens < this.budget.outputTokens && project.cost < this.budget.cost && project.wallMs < this.budget.wallMs && session.calls < this.budget.sessionCalls; }); }
  ...
  async run(job:LcmJob, model:LcmSummarizer, input:string, signal=new AbortController().signal): Promise<LcmNode> {
    ...
    try {
      ...
      for (const [index, level] of LCM_MODEL_LEVELS.entries()) {
        ...
        if (index > 0 && !this.withinBudget(sessionId)) break;
      ...
      catch { try { this.fail(claimed, String(error)); } catch {} throw error; }
