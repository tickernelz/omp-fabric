...
Fabric resolves the owner by scanning `AgentRegistry` for the live agent whose session id matches the identity the provider captured, then takes that agent's own `asyncJobManager` (never the process singleton, which the host withholds from secondary sessions on purpose). The owner id is re-resolved per call, so `/new` and a session switch keep delivering. Where no such agent exists, or where the host gave it no manager, `async.enabled` is forced off: the `async` parameter leaves the advertised schema and the tool description promises no backgrounding.
