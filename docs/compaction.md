...
Every summary a compaction serves carries engine-derived addresses beneath each node's text: `address: lcm.summary:<nodeId>` for the node itself, then `sources: lcm.raw:<session>:<entry>:<revision>` for the raw entries it covers, or `children: lcm.summary:<nodeId>` for a condensed node. These are the exact forms `memory.expand` parses, so a model reading a summary can always retrieve what was summarized. They are appended after the frontier walk from ledger data and are never taken from model output. When every block fits, each is rendered complete with its own address. When they do not, the render shares the budget: each node keeps its own text up to an equal share, the nodes under that share keep everything, and the leftover is redistributed to the ones above it, so a 65 KB frontier still reaches the model as ten addressed blocks where it used to reach it as two. A clip's `…` marker sits inside the block text, above its `address:` line. The withheld notice remains for the case where the share would fall under 512 bytes. The withheld notice names what was dropped: `… withheld N frontier nodes that did not fit; expand: lcm.summary:<id>`, listing the addresses it withheld and appending `, +K more` when even that list must truncate. An address list is emitted only when it fits whole, so `sources:` and `children:` lines are never half-written.

A leaf counts as oversized when it carries more sources than `max(lcmMaxLeafEntries, 32)`, so lowering that setting cannot mark ordinary leaves oversized while a legacy whole-tail leaf still trips it.

An address list keeps the newest addresses and names the older ones it dropped, as `sources: +N older, lcm.raw:…`, because recovery reaches for recent evidence first. In a deterministic excerpt that list is capped at a tenth of the output bound, so provenance never crowds out the text it addresses.

The tail order is fixed: the active request block, then block text, the block's `address:` line, its `sources:` or `children:` lines, then the withheld notice when one is needed, then the recent-detail digest, then the recovery pointer. Every non-empty render closes on exactly one recovery pointer line, and a withheld notice sits directly above that pointer with a blank line between them. The render is empty only when no frontier node carries text at all. Every node stays reachable through `memory.expand`; before this, a frontier past 32 KiB lost both the trailing addresses and the pointer.

## Maintenance throughput

Maintenance runs after every turn inside a long agent run, at `agent_end`, and on session compaction, whenever a pass is due; the hook itself never calls a model. A turn arriving while a run is in flight starts no second run. Each run makes at most `compaction.lcmMaintenancePasses` passes bounded by `compaction.lcmMaintenanceRunSeconds` (60 by default, 10 to 600), and a pass opens up to `compaction.lcmMaintenanceConcurrency` leaves, each packing unconsumed raw entries until either `compaction.lcmMaxLeafEntries` or the `compaction.lcmMaxInputChars` prompt budget is reached, so a leaf never clips its own evidence. One pass therefore covers passes times concurrency leaves worth of backlog. An emergency node whose sources no longer fit one prompt is left as it is: upgrading it would summarize a truncated slice and claim coverage of the whole range.

Raise `lcmMaintenancePasses` (1-64) for more leaves per turn. `lcmMaintenanceConcurrency` (1-8, 3 by default) is how many summary calls one run keeps in flight and how many leaves a pass opens, and it decides how much of a backlog a single run can clear. Measured against an 8-job session backlog at 15 seconds per call, a run cleared 4 jobs per turn at 1, and all 8 in one turn at 2 and at 3. Spend per turn scales with the same number, so 3 costs up to three times what 1 costs over the same wall time. A job whose call outlives the run deadline holds its slot until it returns; the deadline stops new dispatch and never abandons work already paid for. Model spend carries no limit by default: `lcmMaxDailyModelCalls`, `lcmMaxSessionModelCalls`, and `lcmMaxDailyModelSeconds` are 0, which means no cap, and setting any of them applies that cap. A summary call is abandoned after `lcmModelTimeoutSeconds` (120 by default, 10 to 900). A deterministic excerpt is written when the summary model cannot be called, when it overruns that deadline, or when `lcmModelSummaries` is false, and maintenance replaces such a node with a model summary once the model answers again. It is separate from `lcmMaxCondenseChildren`, which is the condensation fan-in: how many ready nodes are folded into one condensed parent. The daily project and per-session call, token, cost, and wall-time budgets still bound every run. A claim reserves one call for itself and one for every live sibling lease, so admitted calls stay inside the daily and session call caps. For the token, cost, and wall-time caps it reserves the average consumption per call recorded today. Two cases pay for a call that will be refused: the first calls of a day, where nothing is recorded and the average reserves zero, so the batch is admitted on recorded usage alone, and a call heavier than the recorded mean, which the average understates. In both the call is made, completion refuses its result, and the range falls back to the deterministic excerpt. Recorded usage never passes a cap, because completion re-checks every dimension and refuses a result that would cross it. A lease counts against the cap and against those reservations until 60 seconds after it lapses, the moment the sweep hands its slot back. A worker whose process stalls through the full 90 seconds of lease and reclaim window keeps its call running after its slot is handed on, and its result is refused when it returns: the call is paid for, no usage is recorded, and the job keeps its attempt count. Concurrent calls pass the cap by the number of stalled workers.

## Cutting a turn in half

A compaction can cut inside a turn, leaving the user request that started it behind the boundary. The summary then opens with that request verbatim under `[Active Request] lcm.raw:<session>:<entry>:<revision>`, so the task in flight survives the cut. The block appears only for a split turn: the host's own `isSplitTurn` decides when it is passed, and otherwise the cut is read as split when the first kept entry is not a user message. A cut at a turn boundary carries no such block, because the request before it is already answered. The block is derived from the session branch, never from model output, and the pure fallback path gives it up to 40% of `lcmMaxOutputChars`.

Coverage is partial, not all-or-nothing. Ready summaries render for the range they cover, and the entries no summary covers yet render as `[Recent Detail]`: one line per entry, newest first, carrying user and assistant text, tool calls with their intent, and tool results with their status. The emergency leaf is created over that uncovered tail alone, so it can never grow larger than the model summaries beside it and hide them on the next frontier walk.

## Entry identity

A stored entry keeps one identity across the rewrites the host performs after the fact. Inline image data is hashed to `blob:sha256:<digest>` so the same picture is one entry before and after the host externalizes it; opaque reasoning signatures, `retryRecovery`, and a zero `errorId` are dropped; and a tool result the host replaced with a pruning notice stays on the revision it replaced. Without this an entry that changed only in those fields gained a revision and was summarized a second time.
...
A summary is served only when every source it covers is still on the live branch. That set is rebuilt from the session on every readback, so a rewind drops the summaries that covered the abandoned entries on the next sync, and a fork keeps the ones whose sources it inherited.
