...
describe("LCM session migration", () => {
  ...
  it("keeps structured message content and excludes message_end", () => { const root=make(); const cwd=path.join(root,"project"); fs.mkdirSync(cwd); const p=file(root,cwd,[header(cwd),{type:"message",id:"a",timestamp:"2026-09-07T00:00:00.000Z",parentId:null,message:{role:"assistant",content:[{type:"text",text:"hi"},{type:"image",data:"aGk="}],toolCall:{name:"x"}}},{type:"message_end",id:"end",timestamp:"2026-09-07T00:00:01.000Z",message:{role:"assistant",content:"detached"}}]); const l=openLedger({dbPath:path.join(root,"l.sqlite"),project:{liveCwd:cwd}}); const r=migrateSessions({agentDir:root,ledger:l,files:[p],now:Date.parse("2026-09-08T00:00:00.000Z"),apply:true}); expect(r.counts.imported).toBe(1); const raw = l.readRaw()[0]!; expect(JSON.parse(raw.payloadJson)).toEqual({ type: "message", id: "a", timestamp: "2026-09-07T00:00:00.000Z", parentId: null, message: { role: "assistant", content: [{ type: "text", text: "hi" }, { type: "image", data: "blob:sha256:8f434346648f6b96df89dda901c5176b10a6d83961dd3c1ac88b59b2dc327aa4" }], toolCall: { name: "x" } } }); expect(raw.payloadHash).toBe(raw.contentHash); expect(raw.content).toContain('"toolCall"'); l.close(); });
