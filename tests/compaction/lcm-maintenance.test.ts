...
const keys = (...rows: Array<{ entryId: string; payloadHash: string }>): Set<string> => new Set(rows.map((row) => `${row.entryId}:${row.payloadHash}`));
...
describe("LCM maintenance branch isolation", () => {
  it("selects only summaries whose source refs are on the active branch", async () => {
    ...
    expect(maintenance.getFrontier("s", keys(root, left)).map(n => n.text)).toEqual(["left"]); expect(maintenance.getFrontier("s", keys(root, right)).map(n => n.text)).toEqual(["right"]); ledger.close();
  });
  it("retains shared null ancestors during fork leaf selection", async () => {
    ...
    expect(maintenance.selectLeaf([root, right], keys(root, right)).map(entry => entry.entryId)).toEqual(["root", "right"]); ledger.close();
  });
  it("condenses only the sibling summaries the live branch still holds", async () => {
    ...
    expect(maintenance.selectCondensation("s", keys(root, left)).map(n => n.nodeId)).toEqual([leftNode.nodeId]);
    expect(maintenance.selectCondensation("s", keys(root, right)).map(n => n.nodeId)).toEqual([rightNode.nodeId]);
    ledger.close();
  });
  ...
  it("does not condense nodes from another session", async () => {
    ...
    for (const node of [na, nb]) await maintenance.run(maintenance.listJobs().find(j => j.nodeId === node.nodeId)!, model(result(node.nodeId)), `${node.nodeId} evidence`); expect(maintenance.selectCondensation("s1", keys(a, b)).map(n => n.sessionId)).toEqual(["s1"]); ledger.close();
  });
  ...
  it("abandons a summary call at the configured deadline", async () => {
    ...
    const settled = await maintenance.run(job, hanging, "evidence for the slow call: ".repeat(20));
