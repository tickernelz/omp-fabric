...
const makeRoot = (): string => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "lcm-runtime-"));
  roots.push(root);
  return root;
};
...
describe("LCM runtime", () => {
  it("keys the ledger by recorded project identity", async () => {
    const liveRoot = makeRoot(); const recordedRoot = makeRoot(); const first = new LcmRuntime(makeContext(liveRoot, []), { rootDir: liveRoot }); const secondContext = makeContext(liveRoot, []); secondContext.sessionManager.getRecordedCwd = () => recordedRoot; const second = new LcmRuntime(secondContext, { rootDir: liveRoot });
  ...
  it("rebinds a reused runtime to the newly selected session", async () => {
    ...
    const runtime = new LcmRuntime(context, { rootDir: root }); await runtime.reconcileSelectedSession(); sessionId = "session-2"; sessionFile = secondFile; branch = [secondEntry]; await runtime.reconcileSelectedSession();
  ...
  it("keeps identical source ranges distinct across branches", async () => {
    const root = makeRoot(); const entry = makeEntry("shared", "shared source"); const runtime = new LcmRuntime(makeContext(root, [entry]), { rootDir: root });
  ...
  it("uses the historical branch payload revision", async () => {
    ...
    const runtime = new LcmRuntime(makeContext(root, [historical]), { rootDir: root });
  ...
  it("falls back when the ready frontier covers only part of the source range", async () => {
    const root = makeRoot(); const entries = [makeEntry("old", "old source"), makeEntry("new", "new source", "old")]; const runtime = new LcmRuntime(makeContext(root, entries), { rootDir: root }); await runtime.readback();
  ...
  it("persists new compaction entries before emitting provenance", async () => {
    ...
    const runtime = new LcmRuntime(makeContext(root, entries), { rootDir: root, lcmMaxOutputChars: 1_024 });
    const result = runtime.compact({ branchEntries: entries, sessionId: "session-1", branch: "branch-a", firstKeptEntryId: "missing", tokensBefore: 100 });
    ...
    await runtime.shutdown();
    const restarted = new LcmRuntime(makeContext(root, []), { rootDir: root });
  ...
  it("readbacks authoritative branch entries and returns a bounded emergency result", async () => {
    ...
    const runtime = new LcmRuntime(makeContext(root, entries), { rootDir: root });
    await runtime.readback();
    const rows = runtime.raw("session-1");
  ...
  it("passes the recorded source revision to maintenance", async () => {
    ...
    const runtime = new LcmRuntime(context, { rootDir: root, maxCondenseChildren: 1 });
    await runtime.readback();
